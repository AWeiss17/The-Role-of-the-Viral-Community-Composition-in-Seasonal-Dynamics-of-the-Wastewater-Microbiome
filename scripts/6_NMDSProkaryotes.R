...
#####################################
# select Only Genera, which are in data_rarefied_RNA
...
# rename genera columns to their orders
...
stress <- c()
for (i in 1:4) {
  stress <- c(stress, metaMDS(Prok_data_rel, dist="bray", k = i)$stress)
}
...
# Save stress plot
png("Prok_NMDS_StressPlot.png", width = 800, height = 600, res = 150)
...
# Carry out NMDS
NMDS <- metaMDS(Prok_data_rel, dist="bray", k=2, maxit=1000, trymax=100, autotransform = FALSE)
...
# extract NMDS-Coordinaten 
NMDS_Prok_df <- as.data.frame(scores(NMDS, display = "sites")) %>%
  rownames_to_column("SRR_ID")  # SRR_ID als Spalte hinzufügen

# Save NMDS-Coordinaten as CSV
write_csv(NMDS_Prok_df, "NMDS_Prok_Coordinates.csv")
#####################################
...
# match rownames
Environmental_Data_RNA <- Environmental_Data_RNA[match(rownames(Prok_data_rel), rownames(Environmental_Data_RNA)), ]

# check rownames
identical(rownames(Prok_data_rel), rownames(Environmental_Data_RNA)) 
...
# new Labels in `significant_env_vectors`-Table
significant_env_vectors$Feature_renamed <- label_mapping[significant_env_vectors$Feature]

# save original name in  `label_mapping` if a feature is not in
...
# Extract env vectors from envfit
...
ggplot(nmds_points, aes(x = NMDS1, y = NMDS2, color = Season)) +
  ...
  theme_minimal() +
  labs(title = "Prokaryota", x = "NMDS1", y = "NMDS2") +
  theme(
...
# Filter count_data_silva, save orders
genus_to_order_silva_filtered <- count_data_silva %>%
  filter(Genus %in% prokaryote_genera_present) %>%
  select(Genus, Order) %>%
  distinct()

# extract relevant genera RNA-Tabelle
data_prokaryote <- data_rarefied_RNA %>%
  select(SRR_ID, any_of(prokaryote_genera_present))

# rename Genera column to their orders
genus_to_order_map <- setNames(genus_to_order_silva_filtered$Order, genus_to_order_silva_filtered$Genus)
...
# Filter count_data_silva to save only relevant genera
genus_to_order_silva_filtered <- count_data_silva %>%
  filter(Genus %in% prokaryote_genera_present) %>%
  select(Genus, Order) %>%
  distinct()
...
# rename genera coluns to their orders
genus_to_order_map <- setNames(genus_to_order_silva_filtered$Order, genus_to_order_silva_filtered$Genus)
...
# save only significante Taxa (p < 0.05)
...
# select only neccessary Metadata columns
Environmental_Data_RNA <- Environmental_Data_RNA %>%
  select(SRR_ID, Date, Season, pH_online) 
...
#data_prokaryote_order <- data_prokaryote_order %>% filter(!SRR_ID %in% removed_ARG_samples_RNA)
